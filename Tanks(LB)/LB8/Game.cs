...
namespace LB8
{
    class Game
    {
        ...
        public void Let (Model1 Player, Trees tree, Rock rock, Enemies enemies) // Препятствие
        {
            if (Player.Rideability == true)
            {
                ...
                for (int i = 0; i < rock.Rock_mass.Length; i++)
                {
                    if (Crossing(Player.Player, rock.Rock_mass[i]))
                    { Bias(Player); }
                    else { Player.Rideability = true; }
                    for (int j = 0; j < enemies.Enemies_mass.LongCount(); j++)
                    {
                        if (Crossing(enemies.Enemies_mass[j], rock.Rock_mass[i]))
                        { enemies.Bias(enemies, j); }
                        else { enemies.Rideability = true; }
                    }
                }
